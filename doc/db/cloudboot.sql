...
--
-- Host: 127.0.0.1
-- Generation Time: 2016-04-22 06:14:02
-- 服务器版本： 5.7.10
-- PHP Version: 5.5.27
...
CREATE TABLE IF NOT EXISTS `manufacturers` (
  ...
  `nic` text COMMENT '网卡信息',
  `cpu` text COMMENT 'CPU',
  `memory` text COMMENT '内存',
  `disk` text COMMENT '磁盘',
  `cpu_sum` int(11) DEFAULT '0' COMMENT 'CPU总核数',
  `memory_sum` int(11) DEFAULT '0' COMMENT '内存总容量',
  `disk_sum` int(11) DEFAULT '0' COMMENT '硬盘总容量',
  `motherboard` text COMMENT '主板',
  `raid` text COMMENT 'raid',
  `oob` text COMMENT 'OOB',
  `user_id` int(11) NOT NULL DEFAULT '0'
) ENGINE=InnoDB DEFAULT CHARSET=utf8;
...
ALTER TABLE `vm_devices`
  ADD PRIMARY KEY (`id`), ADD UNIQUE KEY `hostname` (`hostname`), ADD UNIQUE KEY `ip` (`ip`), ADD UNIQUE KEY `mac` (`mac`), ADD KEY `status` (`status`) USING BTREE;
