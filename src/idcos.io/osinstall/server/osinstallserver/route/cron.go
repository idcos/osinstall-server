...
func CloudBootCron(conf *config.Config, logger logger.Logger, repo model.Repo) {
	DBVersionUpdate(conf, logger, repo)
...
`
  ...
  use cloudboot;
delete from platform_configs where name = 'Version';
insert into platform_configs(~~created_at~~,~~updated_at~~,~~name~~,~~content~~) values('2017-01-08 13:32:47','2017-01-08 13:32:47','Version','v1.5');
...
`
	...
	case "v1.4":
		str = v15sql
	default:
		str = v131sql + v14sql + v15sql
	}

	str = strings.Replace(str, "~~", "`", -1)
	file := "/tmp/cloudboot-update.sql"
	bytes := []byte(str)
	...
	cmd := fmt.Sprintf("mysql -uroot -p%s < %s", getPassword(conf.Repo.Connection), file)
...
func getPassword(conn string) string {
	return conn[strings.Index(conn, ":")+1 : strings.Index(conn, "@")]
}
